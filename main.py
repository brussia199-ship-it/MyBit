...
# Проверка доступа к /report (только активная подписка)
async def check_report_access(user_id, message):
    ...
    if not await check_subscription(user_id):
        ...
        await message.reply(
            ...
            f"Для использования команды /report необходимо подписаться на канал {REQUIRED_CHANNEL}\n\n"
    ...
    if not has_active_subscription(user_id):
        ...
        await message.reply(
            ...
            f"• 90 дней — 2500 ⭐\n"
            f"• Навсегда — 5000 ⭐\n\n"
            f"💎 Выберите тариф ниже, чтобы получить доступ к /report:",
            reply_markup=keyboard,
            parse_mode='Markdown'
        )
        return False, None
...
**Пример:** `/report username123`
        ...
        await message.reply(
            f"⚠️ **У вас нет активной подписки!**\n\n"
            f"💰 Выберите тариф ниже для доступа к /report:",
            reply_markup=keyboard,
            parse_mode='Markdown'
        )
    ...
    if await check_subscription(user_id):
        ...
        if has_active_subscription(user_id):
            ...
            await callback.message.edit_text(
                ...
                f"Теперь вы можете использовать /report",
                parse_mode='Markdown'
            )
        else:
            keyboard = get_pricing_keyboard()
            await callback.message.edit_text(
                ...
                f"Теперь приобретите подписку для доступа к /report:",
                reply_markup=keyboard,
                parse_mode='Markdown'
            )
    else:
...
# АДМИН: Проверить подписку пользователя
