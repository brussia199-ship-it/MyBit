...
ADMIN_IDS = [7673683792, 7534265394]  # ЗАМЕНИТЕ НА СВОЙ ID
...
# Функция выдачи подписки админом
...
# Функция забирания подписки админом
...
# Декоратор проверки доступа для /report (только админы)
def require_admin_for_report(handler):
    async def wrapper(message: Message):
        user_id = message.from_user.id
        
        if user_id not in ADMIN_IDS:
            await message.reply("❌ **Доступ запрещён!**\n\nТолько администраторы могут использовать команду /report.", parse_mode='Markdown')
            return
        
        return await handler(message)
    return wrapper

# Декоратор для остальных команд (проверка подписки)
...
**Команды:**
/report <username> - начать отправку жалоб (только для админов)
...
async def cmd_status(message: Message):
    ...
    if user_id not in user_sessions:
        await message.reply("❌ Нет активных или завершённых сессий.", parse_mode='Markdown')
        return
...
async def admin_give_menu(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "Или нажми кнопку ниже для отмены:",
        parse_mode='Markdown'
    )
    await callback.answer()
...
async def admin_give_subscription(message: Message):
    ...
    args = message.text.split()
    if len(args) < 3:
        await message.reply("❌ Формат: `/give user_id количество_дней`\nПример: `/give 123456789 30`", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if days <= 0 or days > 3650:
            await message.reply("❌ Количество дней должно быть от 1 до 3650")
            return
        ...
        try:
            await bot.send_message(
                ...
                f"📅 Действует до: {new_expiry.strftime('%d.%m.%Y')}\n\n"
                f"Используйте /start для проверки.",
                parse_mode='Markdown'
            )
        except:
            pass
        ...
        await message.reply("❌ Ошибка: user_id и количество дней должны быть числами!", parse_mode='Markdown')
...
async def admin_remove_subscription(message: Message):
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.reply("❌ Формат: `/remove user_id`\nПример: `/remove 123456789`", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if remove_subscription(target_user_id):
            ...
            await message.reply(f"❌ У пользователя `{target_user_id}` нет активной подписки!", parse_mode='Markdown')
            
    except ValueError:
        await message.reply("❌ Ошибка: user_id должен быть числом!", parse_mode='Markdown')
...
async def handle_broadcast(message: Message):
    ...
    if user_id in ADMIN_IDS and admin_broadcast_targets.get(user_id, False):
        ...
        for uid in users.keys():
            try:
                if message.text:
                    await bot.send_message(int(uid), message.text, parse_mode='Markdown')
                elif message.photo:
...
async def main():
    ...
    print(f"🔒 /report доступна только админам")
